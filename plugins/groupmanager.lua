...
---------------Lock bad words-------------------
local function lock_bw(msg, data, target) 
local hash = "gp_lang:"..msg.to.id
local lang = redis:get(hash)
if not is_mod(msg) then
if not lang then
...
local lock_bw = data[tostring(target)]["settings"]["lock_bw"] 
if lock_bw == "yes" then
if not lang then
 return "*Bad Words* _Posting Is Already Locked_"
elseif lang then
 return "ارسال کلمات غیر اخلاقی در گروه هم اکنون ممنوع است"
end
else
 data[tostring(target)]["settings"]["lock_bw"] = "yes"
save_data(_config.moderation.data, data) 
if not lang then
 return "*Bad Words* _Posting Has Been Locked_"
else
 return "ارسال کلمات غیر اخلاقی در گروه ممنوع شد"
end
end
end

local function unlock_bw(msg, data, target)
local hash = "gp_lang:"..msg.to.id
local lang = redis:get(hash)
 if not is_mod(msg) then
if not lang then
...
local lock_bw = data[tostring(target)]["settings"]["lock_bw"]
 if lock_bw == "no" then
if not lang then
return "*Bad Words* _Posting Is Not Locked_" 
elseif lang then
return "ارسال کلمات غیر اخلاقی در گروه ممنوع نمیباشد"
end
else 
data[tostring(target)]["settings"]["lock_bw"] = "no" save_data(_config.moderation.data, data) 
if not lang then
return "*Bad Words* _Posting Has Been Unlocked_" 
else
return "ارسال کلمات غیر اخلاقی در گروه آزاد شد"
end
end
end
...
elseif lang then
 return "ارسال ایموجی در گروه هم اکنون ممنوع است"
end
else
...
if not lang then
 ...
 return "ارسال ایموجی در گروه ممنوع شد"
end
end
end
...
elseif lang then
return "ارسال ایموجی در گروه ممنوع نمیباشد"
end
else 
...
return "ارسال ایموجی در گروه آزاد شد"
end
end
end
...
elseif lang then
 return " انگلیسی در گروه هم اکنون ممنوع است"
end
else
...
if not lang then
 ...
 return " زبان انگلیسی در گروه ممنوع شد"
end
end
end
...
elseif lang then
return "تایپ با زبان انگلیسی در گروه ممنوع نمیباشد"
end
else 
...
return " زبان انگلیسی در گروه آزاد شد "
end
end
end
...
elseif lang then
 return "ارسال نام کاربری در گروه هم اکنون ممنوع است"
end
else
...
if data[tostring(target)]["settings"] then		
if not data[tostring(target)]["settings"]["lock_bw"] then			
data[tostring(target)]["settings"]["lock_bw"] = "yes"		
end
end
...
local settings = data[tostring(target)]["settings"] 
 text = "[🛠]*Group Settings  :⤵\n_________________________\n➡Lock* edit              : *"..settings.lock_edit.."*\n-----------------------------------------\n➡*Lock* links            : *"..settings.lock_link.."*\n-----------------------------------------\n➡*Lock* Bad words  : *"..settings.lock_bw.."*\n-----------------------------------------\n➡*Lock* emoji           : *"..settings.lock_emoji.."*\n-----------------------------------------\n➡*Lock* English        : *"..settings.lock_english.."*\n-----------------------------------------\n➡*Lock* Username   : *"..settings.lock_username.."*\n-----------------------------------------\n*➡Lock* tags             : *"..settings.lock_tag.."*\n-----------------------------------------\n➡*Lock* spam           : *"..settings.lock_spam.."*\n-----------------------------------------\n➡*Lock* mention       : *"..settings.lock_mention.."*\n-----------------------------------------\n➡*Lock* arabic          : *"..settings.lock_arabic.."*\n-----------------------------------------\n➡*Lock* webpage     : *"..settings.lock_webpage.."*\n-----------------------------------------\n➡*Lock* markdown   : *"..settings.lock_markdown.."*\n-----------------------------------------\n➡*Bots protection*   : *"..settings.lock_bots.."*\n-----------------------------------------\n🍀*Group welcome*   : *"..settings.welcome.."*\n-----------------------------------------\n🆘*Lock* flood            : *"..settings.flood.."*\n🔢*flood* sensitivity   : *"..NUM_MSG_MAX.."*\n----------------------------------------\n♻*Expire* time :*[ "..expire.." ]* Days\n*________________________\nGroup Language* : *EN\nBot channel*: @ChRFD "
else
local settings = data[tostring(target)]["settings"] 
 text = "توضیح:\n _علامت✅ یعنی قفل مورد نظر فعال است\nعلامت☑️ یعنی آن قفل غیر فعال است_\n*-----------------------------------------\n[🛠]*تنظیمات قفل ها    : ⬇\n*___________lock__________*\n⬅ _ ویرایش پیام_           : *"..settings.lock_edit.."*\n-----------------------------------------\n⬅ _ لینک_                      : *"..settings.lock_link.."*\n-----------------------------------------\n⬅  _کلمات فیلتر شده_      : *"..settings.lock_bw.."*\n-----------------------------------------\n⬅ _ ایموجی_                  : *"..settings.lock_emoji.."*\n-----------------------------------------\n⬅ _ زبان انگلیسی_           : *"..settings.lock_english.."*\n-----------------------------------------\n➡ _ نام کاربری_               : *"..settings.lock_username.."*\n-----------------------------------------\n⬅ _ تگ_                         : *"..settings.lock_tag.."*\n-----------------------------------------\n⬅ _ هرزنامه_                  : *"..settings.lock_spam.."*\n-----------------------------------------\n⬅ _ فراخوانی_                : *"..settings.lock_mention.."*\n-----------------------------------------\n⬅ _ عربی_                      : *"..settings.lock_arabic.."*\n-----------------------------------------\n⬅ _ صفحات وب_            : *"..settings.lock_webpage.."*\n-----------------------------------------\n⬅ _ فونت_                     : *"..settings.lock_markdown.."*\n-----------------------------------------\n⬅ _ضد ربات_                 : *"..settings.lock_bots.."*\n-----------------------------------------\n⬅ _ پیام مکرر_                : *"..settings.flood.."*\n⬅ _حداکثر پیام مکرر_       : *"..NUM_MSG_MAX.."*\n-----------------------------------------\n🍀 _پیام خوشآمد گویی_   : *"..settings.welcome.."*\n-----------------------------------------\n⬅ _تاریخ انقضا ربات_   : *[ "..expire.." ]* _ روز_\n*________________________*\n⬅ _زبان سوپرگروه_ : *FA*\n📰 _کانال ما_ : @ChRFD\n----------------------------------------\n_جهت در یافت سایر تنظیمات بروی دستور زیر بزنید_\n/mutelist "
end
...
local mutes = data[tostring(target)]["mutes"] 
 text = "توضیح:\n _علامت✅ یعنی قفل مورد نظر فعال است\nعلامت☑️ یعنی آن قفل غیر فعال است_\n-----------------------------------------\n *لیست قفل ها رسانه*    : ⬇\n*________________________*\n_⬅ قفل گروه بطور کل_ : *"..mutes.mute_all.."*\n-----------------------------------------\n_⬅ تصاویر متحرک_      : *"..mutes.mute_gif.."*\n-----------------------------------------\n_⬅ متن_                      : *"..mutes.mute_text.."*\n-----------------------------------------\n_⬅ کیبورد شیشه ای_    : *"..mutes.mute_inline.."*\n-----------------------------------------\n_⬅ بازی های تحت وب_ : *"..mutes.mute_game.."*\n-----------------------------------------\n_⬅ عکس_                   : *"..mutes.mute_photo.."*\n-----------------------------------------\n_⬅ فیلم_                     : *"..mutes.mute_video.."*\n-----------------------------------------\n_⬅ آهنگ_                    : *"..mutes.mute_audio.."*\n-----------------------------------------\n_⬅ صدا_                     : *"..mutes.mute_voice.."*\n-----------------------------------------\n_⬅ برچسب_                : *"..mutes.mute_sticker.."*\n-----------------------------------------\n_⬅ مخاطب_                : *"..mutes.mute_contact.."*\n-----------------------------------------\n_⬅ نقل قول_                : *"..mutes.mute_forward.."*\n-----------------------------------------\n_⬅ موقعیت_                : *"..mutes.mute_location.."*\n-----------------------------------------\n_⬅ اسناد_                    : *"..mutes.mute_document.."*\n-----------------------------------------\n_⬅ خدمات تلگرام_        : *"..mutes.mute_tgservice.."*\n-----------------------------------------\n_⬅ صفحه کلید_            : *"..mutes.mute_keyboard.."*\n*________________________*\n_کانال ما_: @ChRFD \n_زبان سوپرگروه_ : *فارسی*\n-----------------------------------------\n_جهت دریافت سایر تنظیمات بر روی دستور زیر بزنید_\n/settings"
end
text = string.gsub(text, "yes", "✅")
text = string.gsub(text, "no", "☑️")
return text
